...
def run_spb():
    soup = BeautifulSoup(curl_spb.response.text, "lxml")
    product_id(soup)
    product_name(soup)
    product_link(soup)
    product_price(soup)
    ...
    for number in total_page:
        ...
        page = f"{curl_spb.response.url}?page={number_of_page}"
        response = requests.get(
            page,
            cookies=curl_spb.cookies,
            headers=curl_spb.headers,
        )
...
def save(data_id, data_name, data_link, data_price, data_brand):
    for index, item in enumerate(data_id):
        ...
        with open("spb.json", "a", encoding="utf-8") as file:
